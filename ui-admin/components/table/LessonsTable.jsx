...
const lessonsDataInitial = [
    {
        _id: "LESSON101",
        courseId: "COURSE001",
        title: "Variables & Data Types",
        content: "Text + Code snippet...",
        videoUrl: "https://video.com/lesson101.mp4",
        quizIds: ["QUIZ001", "QUIZ002"],
...
const quizz = [
    {
        ...
        options: [
            ...
            { option: "print('Hello World')", correct: true },
            { option: "print('Hello World')", correct: false },
            { option: "print('Hello World')", correct: false }
        ],
        createdAt: "2024-12-31T23:59:59Z"
    },
    {
        ...
        options: [
            ...
            { option: "Dictionary", correct: false },
            { option: "Tuple", correct: true },
        ...
        <div className="border border-gray-300 rounded-lg p-4 bg-white mb-4">
            ...
            {quiz.options.map((opt, i) => (
                ...
                <label className="flex items-center gap-1 text-xs">
                    <input type="radio" checked={opt.correct} onChange={() => handleCorrectChange(i)} />
                    Đúng
                </label>
                </div>
            ))}
...
const LessonsTable = () => {
    ...
    const [lessonsData, setLessonsData] = useState(lessonsDataInitial);
    ...
    useEffect(() => {
        if (editingLesson) {
        const quizData = editingLesson.quizIds.map(id => quizz.find(q => q._id === id)).filter(Boolean);
        setLessonQuizz(quizData);
        }
    }, [editingLesson]);

    const handleSave = () => {
        setLessonsData(lessonsData.map(l => l._id === editingLesson._id ? { ...editingLesson, quizIds: lessonQuizz.map(q => q._id) } : l));
            ...
            <table className="w-full table-auto text-sm text-left border-collapse">
                ...
                <motion.tbody variants={containerVariants} initial="hidden" animate="visible">
                    {filteredLessons.map(lesson => (
                        <motion.tr key={lesson._id} variants={rowVariants} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                            <td className="py-3 px-4">{lesson._id}</td>
                            <td className="py-3 px-4">{lesson.courseId}</td>
                            <td className="py-3 px-4">{lesson.title}</td>
                            <td className="py-3 px-4">
                                {lesson.videoUrl ? <a href={lesson.videoUrl} target="_blank" rel="noreferrer" className="text-indigo-600 underline">Xem video</a> : "-"}
                            </td>
                            <td className="py-3 px-4 text-center">{lesson.quizIds.length}</td>
                            <td className="py-3 px-4 text-gray-600 whitespace-nowrap">{formatDate(lesson.createdAt)}</td>
            ...
            <div className="bg-white rounded-xl shadow-lg w-full max-w-4xl p-6 relative">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    {editingLesson.videoUrl && (
                        <video src={editingLesson.videoUrl} controls className="w-full h-60 rounded-lg border mt-2" />
                    )}
                </div>
                    ...
                    {lessonQuizz.map((quizItem, idx) => (
                        <QuizItem
                            ...
                            onChange={(updatedQuiz) => {
                                const newQuizArr = [...lessonQuizz];
                                newQuizArr[idx] = updatedQuiz;
                                setLessonQuizz(newQuizArr);
                            }}
                        />
                    ))}
                </div>
